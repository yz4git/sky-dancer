...
test("V15 Orbital Ascent breaks the repeated concentric tunnel rhythm", () => {
  assert.deepEqual(
    Array.from({ length: 8 }, (_, index) => skyDancerArcadeV15OrbitChunkVisible(index)),
    [true,false,false,false,true,false,false,false],
  );
  assert.deepEqual(
    Array.from({ length: 10 }, (_, index) => skyDancerArcadeV15OrbitCueVisible(index)),
  ...
  const root = scene.getObjectByName("arcade-course-environment")!;
  const chunks = root.children.filter((object) => object.name.startsWith("arcade-course-chunk-"));
  assert.equal(chunks.filter((chunk) => chunk.visible).length, 2);

  const cues = scene.getObjectsByProperty("name", "arcade-orbit-helix-cue");
  const visibleCues = cues.filter((cue) => cue.visible);
  assert.equal(visibleCues.length, 3, "ten authored cues become three open-space ascent beats");
  for (const cue of visibleCues) {
    ...
    assert.ok(Math.abs(visibleTori[0].position.x) >= 12, "orbital helix arcs stay offset from the hero line");
    assert.ok(visibleTori[0].scale.x <= .8, "orbital helix arcs stay compact enough to leave open space");
  }
  environment.dispose();
});
