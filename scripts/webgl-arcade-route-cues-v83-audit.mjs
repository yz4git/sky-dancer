...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-route-cues-v83";
const stages = [
...
for (const [index, stage] of stages.entries()) {
  ...
  const shot = async (suffix) => {
    ...
    console.log(`[v83-audit] ${stage.id}: ${suffix}`);
  };
...
console.log(`[v83-audit] complete: ${diagnostics.length} stages / 8 captures`);
