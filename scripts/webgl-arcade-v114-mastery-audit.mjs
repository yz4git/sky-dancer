...
const out = "artifacts/v114-mastery";
...
try {
  ...
  const menuGeometry = await page.evaluate(() => {
    ...
    return { mastery: rect(masteryEl), start: rect(startEl), innerWidth, innerHeight, scrollWidth: document.documentElement.scrollWidth, scrollHeight: document.documentElement.scrollHeight };
  });
  ...
  await page.waitForFunction(() => Boolean(window.__skyDancerV114AuditDemo));
  await page.waitForTimeout(220);
  const flightTelemetry = await page.evaluate(() => {
    const demo = window.__skyDancerV114AuditDemo;
...
if (report.failures.length) throw new Error(`V11.4 mastery audit failures: ${JSON.stringify(report.failures)}`);
