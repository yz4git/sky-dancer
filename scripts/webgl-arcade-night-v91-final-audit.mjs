...
const outputDir=process.env.SKY_DANCER_AUDIT_DIR||"artifacts/arcade-night-v91-final";
...
for(let i=0;i<await practice.count();i++){const b=practice.nth(i);if((await b.locator("strong").textContent())?.trim()==="METRO"){target=b;break;}}
if(!target)throw new Error("METRO practice stage not found");await target.click();
...
const captures=[];const shot=async(label)=>{await canvas.screenshot({path:`${outputDir}/${label}.png`,timeout:60_000});captures.push(label);console.log(`[v91-night-final] ${label}`);};
await page.waitForTimeout(900);await shot("00-entry");
for(const [delay,label] of [[1700,"01-pursuit-a"],[1800,"02-pursuit-b"],[1900,"03-pursuit-c"],[2000,"04-pursuit-d"]]){await page.waitForTimeout(delay);await shot(label);}
...
const diagnostics={stageVisible:body.includes("NIGHT METRO"),hp,glState,captures,blockingConsoleErrors,pageErrors,failed:/AIRFRAME LOST|MISSION FAILED/i.test(body)};
...
console.log(`[v91-night-final] complete / hp ${hp}`);
