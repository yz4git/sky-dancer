...
const outputDir=process.env.SKY_DANCER_AUDIT_DIR||"artifacts/arcade-cloud-v92-final";
...
for(let i=0;i<await practice.count();i++){const b=practice.nth(i);if((await b.locator("strong").textContent())?.trim()==="FLEET"){target=b;break;}}
if(!target)throw new Error("FLEET practice stage not found");await target.click();
...
const captures=[];const shot=async(label)=>{await canvas.screenshot({path:`${outputDir}/${label}.png`,timeout:60_000});captures.push(label);console.log(`[v92-cloud-final] ${label}`);};
await page.waitForTimeout(900);await shot("00-entry");
for(const [delay,label] of [[1700,"01-armada-a"],[1800,"02-armada-b"],[1900,"03-armada-c"],[2000,"04-armada-d"]]){await page.waitForTimeout(delay);await shot(label);}
...
const diagnostics={stageVisible:body.includes("CLOUD FLEET"),hp,glState,captures,blockingConsoleErrors,pageErrors,failed:/AIRFRAME LOST|MISSION FAILED/i.test(body)};
...
console.log(`[v92-cloud-final] complete / hp ${hp}`);
