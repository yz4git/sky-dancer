...
import { chromium } from '../.audit-runtime/node_modules/playwright-core/index.js';
...
await context.addInitScript(() => {
  ...
  localStorage.setItem('sky-dancer-arcade-progress-v2', JSON.stringify({
    ...
    totalBossKills: 11,
    totalArmorBreaks: 20,
    totalFormationBreaks: 14,
    bestChain: 12,
    bestRoute: ['dawn-city','red-canyon','storm-carrier','ice-cavern','night-metro','orbital-ascent','prism-citadel'],
    bestRouteScore: 180000,
...
async function metrics(label) {
  return await page.evaluate((label) => {
    ...
    const canvas = document.querySelector('canvas.sky-dancer-arcade-canvas');
    const stick = document.querySelector('[role="application"][aria-label="Flight stick"]');
    const fire = byText('button','FIRE');
    const lock = byText('button','LOCK');
    const turbo = byText('button','TURBO');
    const renderer = [...document.querySelectorAll('span')].find((el) => (el.textContent || '').includes('3D FLIGHT') || (el.textContent || '').includes('COMPATIBILITY'));
    return {
      label,
      ...
      canvas: rect(canvas), stick: rect(stick), fire: rect(fire), lock: rect(lock), turbo: rect(turbo),
      renderer: renderer?.textContent || null,
      stageText: document.querySelector('header strong')?.textContent || null,
...
for (const [label, shortName] of cases) {
  await openMenu();
  await page.getByRole('button', { name: /STAGE PRACTICE/i }).click();
  const stageButton = page.getByRole('button', { name: new RegExp(shortName, 'i') }).first();
  ...
  await screenshot(`${label}-01`);
  await playPattern();
  const after = await metrics(`${label}-after`);
  await screenshot(`${label}-02`);
  report.cases.push({ label, shortName, before, after });
}
