...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-v89-triple";
...
const stages = [
  { shortName: "RUINS", slug: "floating-ruins", name: "FLOATING RUINS" },
  { shortName: "METRO", slug: "night-metro", name: "NIGHT METRO" },
  { shortName: "CITADEL", slug: "prism-citadel", name: "PRISM CITADEL" },
];
...
for (const stage of stages) {
  const stageDir = `${outputDir}/${stage.slug}`;
  ...
  for(let i=0;i<await practice.count();i++){
    ...
    if((await button.locator("strong").textContent())?.trim()===stage.shortName){ target=button; break; }
  }
  if(!target) throw new Error(`${stage.shortName} practice stage not found`);
  await target.click();
  ...
  const shot=async(label)=>{
    const path=`${stageDir}/${label}.png`;
    await canvas.screenshot({path,timeout:60_000});
    captures.push(label);
    console.log(`[v89-${stage.slug}] ${label}`);
  };
  await page.waitForTimeout(900); await shot("00-entry");
  for(const [delay,label] of [[1700,"01-a"],[1800,"02-b"],[1900,"03-c"],[2000,"04-d"]]){
  ...
  const entry={stage:stage.slug,stageVisible:body.includes(stage.name),hp,glState,captures,blockingConsoleErrors,pageErrors,failed:/AIRFRAME LOST|MISSION FAILED/i.test(body)};
  diagnostics.push(entry);
  if(!entry.stageVisible||!glState.webgl||glState.width<800||glState.height<360)throw new Error(`Invalid render: ${JSON.stringify(entry)}`);
  if(hp<=0||entry.failed)throw new Error(`Airframe lost: ${JSON.stringify(entry)}`);
  if(captures.length!==6)throw new Error(`Missing captures: ${JSON.stringify(entry)}`);
  if(blockingConsoleErrors.length||pageErrors.length)throw new Error(`Browser errors: ${JSON.stringify(entry)}`);
  await page.close();
}
await writeFile(`${outputDir}/diagnostics.json`,JSON.stringify(diagnostics,null,2));
await browser.close();
console.log(`[v89-triple] complete ${diagnostics.map(d=>`${d.stage}:${d.hp}`).join(" / ")}`);
