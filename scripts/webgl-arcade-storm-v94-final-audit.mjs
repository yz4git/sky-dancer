...
const outputDir=process.env.SKY_DANCER_AUDIT_DIR||"artifacts/arcade-storm-v94-final";
...
const captures=[];const shot=async(label)=>{await canvas.screenshot({path:`${outputDir}/${label}.png`,timeout:60_000});captures.push(label);console.log(`[v94-storm-final] ${label}`);};
await page.waitForTimeout(900);await shot("00-entry");
for(const [delay,label] of [[1700,"01-dreadnought-a"],[1800,"02-dreadnought-b"],[1900,"03-lightning-c"],[2000,"04-dreadnought-d"]]){await page.waitForTimeout(delay);await shot(label);}
...
console.log(`[v94-storm-final] complete / hp ${hp}`);
