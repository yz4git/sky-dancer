...
edit("src/sky/SkyDancerSkyRaid.ts", (source) => {
  ...
  if (!next.includes("enemyGroups: Map<string, THREE.Group>")) {
    next = next.replace(
  ...
  if (!next.includes("const enemyAltitudeReference = flight.altitude;")) {
    next = next.replace(
      ...
      `  (demo.session as unknown as { skyDancerPlayerAltitudeMeters?: number }).skyDancerPlayerAltitudeMeters = flight.altitude;\n  // Center the enemy engagement layer on the player at every SKY RAID altitude.\n  // V43 still supplies local vertical dogfight motion, compressed for phone framing.\n  const enemyAltitudeReference = flight.altitude;\n  setSkyDancerEnemyAltitudeReferenceV56(enemyAltitudeReference);\n  demo.scene.userData.skyRaidEnemyAltitudeReference = enemyAltitudeReference;\n  demo.scene.userData.skyRaidPlayerAltitude = flight.altitude;`,
    );
  }
  ...
  if (!next.includes("applySkyRaidEnemyFlightBand(demo);")) {
    next = next.replace(
...
edit("scripts/webgl-sky-raid-camera-edge-v17.mjs", (source) => {
  ...
  if (!next.includes("baseline.enemyVisible ?? 0")) {
    next = next.replace(
  ...
  if (!next.includes("high.enemyVisible ?? 0")) {
    next = next.replace(
...
console.log("Applied idempotent SKY RAID all-altitude enemy presence and visual feedback pass");
