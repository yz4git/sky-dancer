...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-night-v91";
...
for(let i=0;i<await practice.count();i++){
  ...
  if((await button.locator("strong").textContent())?.trim()==="METRO"){target=button;break;}
}
if(!target)throw new Error("METRO practice stage not found");
await target.click();
...
const captures=[];
const shot=async(label)=>{const path=`${outputDir}/${label}.png`;await canvas.screenshot({path,timeout:60_000});captures.push(label);console.log(`[v91-night] ${label}`);};
await page.waitForTimeout(900);await shot("00-entry");
for(const [delay,label] of [[1700,"01-chicane-a"],[1800,"02-chicane-b"],[1900,"03-chicane-c"],[2000,"04-chicane-d"]]){await page.waitForTimeout(delay);await shot(label);}
...
console.log(`[v91-night] complete / hp ${hp}`);
