...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-prism-v89";
...
for(let i=0;i<await practice.count();i++){
  ...
  if((await button.locator("strong").textContent())?.trim()==="CITADEL"){target=button;break;}
}
if(!target)throw new Error("CITADEL practice stage not found");
await target.click();
...
const shot=async(label)=>{
  ...
  captures.push(label);
  console.log(`[v89-prism] ${label}`);
};
await page.waitForTimeout(900); await shot("00-entry");
for(const [delay,label] of [[1700,"01-assault-a"],[1800,"02-assault-b"],[1900,"03-assault-c"],[2000,"04-assault-d"]]){
...
const diagnostics={stageVisible:body.includes("PRISM CITADEL"),hp,glState,captures,blockingConsoleErrors,pageErrors,failed:/AIRFRAME LOST|MISSION FAILED/i.test(body)};
...
console.log(`[v89-prism] complete / hp ${hp}`);
