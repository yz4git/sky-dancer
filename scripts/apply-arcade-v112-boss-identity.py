...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    s = p.read_text()
    if old not in s:
        raise SystemExit(f"missing patch needle in {path}: {old[:140]!r}")
    p.write_text(s.replace(old, new, 1))

bosses = r'''import type {
...
'''
Path("src/sky/arcade/SkyDancerArcadeV11Bosses.ts").write_text(bosses)
...
'''    this.bossSpawned = false;\n    this.bossDefeated = false;\n    this.stageStats.scoreAtStart = this.score;\n''')
# spawnBoss: make the initial mechanic explicit and serializable.
replace_once(runtime,
'''    this.message = `WARNING · ${this.stage.bossName}`;\n    this.messageTimer = 3.2;\n    this.stageSerial += 1;\n  }\n\n  private spawnHazardPattern''',
...
# Keep the deterministic boss test hook but expose a phase-specific helper as well.
replace_once(runtime,
...
# Remove now-unused V10 boss phase label import if no longer referenced.
p = Path(runtime)
s = p.read_text().replace('  skyDancerArcadeBossPhaseLabel,\n', '')
p.write_text(s)
...
s = css.read_text()
s += '\n.bossHud strong em{display:block;margin-top:2px;font-style:normal;font-size:6px;font-weight:1000;letter-spacing:.16em;color:#ffd77b;text-shadow:0 0 9px rgba(255,190,83,.3)}\n'
css.write_text(s)

# Audio punctuation on boss mechanic changes.
...
'''
Path("tests/sky-arcade-v112-bosses.test.ts").write_text(tests)

p = Path("docs/ARCADE_V11_EVOLUTION.md")
s = p.read_text() + r'''

## V11.2 — Boss Identity
- All 11 climax targets now own a unique motion style instead of sharing one sinusoidal carrier pattern.
- Each phase has a named mechanic, depth profile, pursuit weight, weakpoint cadence, projectile cadence/spread/guidance and intensity.
...
print("V11.2 boss identity applied")
