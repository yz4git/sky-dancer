...
rep(runtime, '''      amplitude: 0.12 + this.random() * 0.38,''', '''      amplitude: 0.28 + this.random() * 0.72,''')
rep(runtime, '''      amplitude: 0.72,''', '''      amplitude: 1.42,''')
rep(runtime,
...
p = Path(runtime)
text = p.read_text()
old = '''      vx: target ? (target.x - this.playerX) * 0.48 : 0,
      vy: target ? (target.y - this.playerY) * 0.48 : 0,
      nearMissChecked: false,'''
new = '''      vx: target ? (target.x - this.playerX) * 0.48 : 0,
      vy: target ? (target.y - this.playerY) * 0.48 : 0,
      guidance: 0,
      nearMissChecked: false,'''
if text.count(old) != 1:
    raise SystemExit('player gun constructor mismatch')
...
new = '''        vx: 0,
        ...
        nearMissChecked: false,'''
if text.count(old) != 1:
    raise SystemExit('player missile constructor mismatch')
text = text.replace(old, new, 1)
p.write_text(text)
rep(runtime,
...
Path(camera).write_text('''/** Wide-field chase camera: the player can traverse about two previous view-widths/heights while the camera follows without pinning the craft to screen center. */
...
addition = '''test("wide-field steering traverses roughly two legacy screens and reverses quickly", () => {
