...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-missile-v96";
...
await context.addInitScript((ids) => localStorage.setItem("sky-dancer-arcade-progress-v1", JSON.stringify({
  version: 1, clearedStageIds: ids, unlockedStageIds: ids, records: {}, bestRunScore: 0, bestRunRank: "D", completedRuns: 0, oneCreditClears: 0,
})), allStageIds);
...
await page.waitForFunction(() => {
  const b = [...document.querySelectorAll('[aria-label="Select game mode"] button')].find((c) => c.textContent?.includes("STAGE PRACTICE"));
...
for (let i = 0; i < await practice.count(); i++) {
  const b = practice.nth(i);
  if ((await b.locator("strong").textContent())?.trim() === "CITY") { target = b; break; }
}
if (!target) throw new Error("CITY practice stage not found");
await target.click();
...
const captures = [];
const shot = async (label) => { await canvas.screenshot({ path: `${outputDir}/${label}.png`, timeout: 60_000 }); captures.push(label); console.log(`[v96-missile] ${label}`); };

await page.waitForTimeout(1200);
await shot("00-entry");
const lock = page.locator('[aria-label="Arcade combat controls"] button').filter({ hasText: /LOCK/i }).first();
await lock.waitFor({ state: "visible" });
const box = await lock.boundingBox();
if (!box) throw new Error("LOCK control has no bounds");
await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
await page.mouse.down();
await page.waitForTimeout(3100);
await shot("01-lock-acquired");
await page.mouse.up();
await page.waitForTimeout(45);
await shot("02-salvo-launch");
await page.waitForTimeout(90);
await shot("03-white-smoke-a");
await page.waitForTimeout(115);
await shot("04-white-smoke-b");
await page.waitForTimeout(165);
await shot("05-white-smoke-tail");
...
const glState = await canvas.evaluate((element) => {
  ...
  return { webgl: Boolean(gl), width: element.getBoundingClientRect().width, height: element.getBoundingClientRect().height, renderer: debug && gl ? gl.getParameter(debug.UNMASKED_RENDERER_WEBGL) : null };
});
const blockingConsoleErrors = consoleErrors.filter((m) => !/Failed to load resource:.*404/i.test(m));
const diagnostics = {
  stageVisible: body.includes("DAWN CITY"), hp, glState, captures,
  lockReadoutPresent: /LOCK\s+\d+\/8/i.test(body),
  blockingConsoleErrors, pageErrors, failed: /AIRFRAME LOST|MISSION FAILED/i.test(body),
};
...
console.log(`[v96-missile] complete / hp ${hp}`);
