...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-ice-v87";
...
for(let i=0;i<await practice.count();i++){
  ...
  if((await button.locator("strong").textContent())?.trim()==="ICE"){ice=button;break;}
}
if(!ice)throw new Error("ICE practice stage not found");
await ice.click();
...
const shot=async(label)=>{
  ...
  captures.push(label);
  console.log(`[v87-ice] ${label}`);
};
await page.waitForTimeout(800); await shot("00-entry");
for(const [delay,label] of [[1800,"01-wave-a"],[1900,"02-wave-b"],[1900,"03-wave-c"],[1900,"04-wave-d"]]){
  await page.waitForTimeout(delay); await shot(label);
}
await page.keyboard.down(" "); await page.waitForTimeout(850); await shot("05-turbo"); await page.keyboard.up(" ");
...
const diagnostics={stageVisible:body.includes("ICE CAVERN"),hp,glState,captures,blockingConsoleErrors,pageErrors,failed:/AIRFRAME LOST|MISSION FAILED/i.test(body)};
...
console.log(`[v87-ice] complete / hp ${hp}`);
