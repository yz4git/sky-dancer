...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-missile-v961";
...
const captures = []; const shot = async (label) => { await canvas.screenshot({ path: `${outputDir}/${label}.png`, timeout: 60000 }); captures.push(label); console.log(`[v961] ${label}`); };
await page.waitForTimeout(500); await page.evaluate(() => window.__skyDancerAuditDemo.pause()); await shot("00-entry");

const setup = await page.evaluate(() => {
  const demo = window.__skyDancerAuditDemo; const runtime = demo.runtime; demo.resume();
  for (let i = 0; i < 190; i++) runtime.step(1 / 60);
  demo.setLock(true); let locked = 0;
  for (let i = 0; i < 420 && locked === 0; i++) {
    const s = runtime.getSnapshot(); locked = s.lockedCount; if (locked) break;
    const candidates = s.enemies.filter((e) => e.depth >= 12 && e.depth <= 82 && !e.locked);
    const e = candidates.sort((a,b) => Math.hypot(a.x-s.playerX,a.y-s.playerY) - Math.hypot(b.x-s.playerX,b.y-s.playerY))[0];
    if (e) demo.setMove(Math.max(-1, Math.min(1, (e.x-s.playerX)*1.55)), Math.max(-1, Math.min(1, (e.y-s.playerY)*1.55)));
    runtime.step(1/60);
  }
  demo.setMove(0,0); const s = runtime.getSnapshot(); demo.pause(); return { locked:s.lockedCount, serial:s.missileSerial, enemies:s.enemies.length, stageTime:s.stageTimeSeconds };
});
if (setup.locked < 1) throw new Error(`Lock failed ${JSON.stringify(setup)}`);
await page.waitForTimeout(70); await shot("01-real-lock");

const launch = await page.evaluate(() => {
  const demo = window.__skyDancerAuditDemo; demo.resume(); demo.setLock(true); demo.setLock(false); const s = demo.getSnapshot(); demo.pause(); return { serial:s.missileSerial, missiles:s.projectiles.filter((p)=>p.owner==="player-missile").length };
});
if (launch.serial <= setup.serial || launch.missiles < 1) throw new Error(`Launch failed ${JSON.stringify({setup,launch})}`);
await page.waitForTimeout(90); await shot("02-launch-plume");

for (const [steps,label] of [[3,"03-white-smoke-a"],[5,"04-white-smoke-b"],[7,"05-white-smoke-tail"]]) {
  await page.evaluate((count) => { const d=window.__skyDancerAuditDemo; d.resume(); for(let i=0;i<count;i++) d.runtime.step(1/60); d.pause(); }, steps);
  await page.waitForTimeout(90); await shot(label);
}
...
const diagnostics = { stageVisible:body.includes("DAWN CITY"), hp, lockedBeforeLaunch:setup.locked, missileSerialBefore:setup.serial, missileSerialAfter:final.missileSerial, launchMissiles:launch.missiles, glState, captures, blockingConsoleErrors, pageErrors, failed:/AIRFRAME LOST|MISSION FAILED/i.test(body) };
await writeFile(`${outputDir}/diagnostics.json`, JSON.stringify(diagnostics,null,2)); await browser.close();
if (!diagnostics.stageVisible || !glState.webgl || hp <= 0 || diagnostics.failed || setup.locked < 1 || launch.missiles < 1 || captures.length !== 6 || blockingConsoleErrors.length || pageErrors.length) throw new Error(`Audit failed ${JSON.stringify(diagnostics)}`);
console.log(`[v961] complete lock=${setup.locked} missile=${setup.serial}->${final.missileSerial} hp=${hp}`);
