...
const outputDir=process.env.SKY_DANCER_AUDIT_DIR||"artifacts/arcade-desert-v93-final";
...
for(let i=0;i<await practice.count();i++){const b=practice.nth(i);if((await b.locator("strong").textContent())?.trim()==="DESERT"){target=b;break;}}
if(!target)throw new Error("DESERT practice stage not found");await target.click();
...
const captures=[];const shot=async(label)=>{await canvas.screenshot({path:`${outputDir}/${label}.png`,timeout:60_000});captures.push(label);console.log(`[v93-desert-final] ${label}`);};
await page.waitForTimeout(900);await shot("00-entry");
for(const [delay,label] of [[1700,"01-sandwall-a"],[1800,"02-breach-b"],[1900,"03-sandwall-c"],[2000,"04-citadel-d"]]){await page.waitForTimeout(delay);await shot(label);}
...
const diagnostics={stageVisible:body.includes("DESERT FORTRESS"),hp,glState,captures,blockingConsoleErrors,pageErrors,failed:/AIRFRAME LOST|MISSION FAILED/i.test(body)};
...
console.log(`[v93-desert-final] complete / hp ${hp}`);
