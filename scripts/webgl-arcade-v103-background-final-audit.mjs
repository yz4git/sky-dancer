...
const outputDir = process.env.SKY_DANCER_AUDIT_DIR || "artifacts/arcade-v103-background-final";
...
await page.waitForFunction(() => Boolean(globalThis.__skyDancerV103FinalAudit), null, { timeout: 30_000 });
...
// Actual play: steering + turbo + fire/lock while scenery streams.
await page.waitForTimeout(900); await shot("00-live-opening-after");
await page.keyboard.down("ArrowRight"); await page.keyboard.down("ArrowUp"); await page.waitForTimeout(900); await shot("01-live-right-climb-after");
await page.keyboard.up("ArrowRight"); await page.keyboard.up("ArrowUp");
await page.keyboard.down(" "); await page.waitForTimeout(800); await shot("02-live-turbo-after"); await page.keyboard.up(" ");
...
await page.waitForTimeout(1100); await shot("03-live-combat-after");
for (const key of ["ArrowLeft","ArrowDown","c","x"]) await page.keyboard.up(key);

const cases = [
  ["dawn-city",.31],["red-canyon",.31],["cloud-fleet",.31],["storm-carrier",.31],["desert-fortress",.31],
  ["ice-cavern",.22],["ice-cavern",.41],["floating-ruins",.31],["night-metro",.31],["volcano-core",.31],["orbital-ascent",.31],["prism-citadel",.31],
];
const captures=[];
let index=10;
for (const [stageId,progress] of cases) {
  const state=await page.evaluate(({stageId,progress})=>globalThis.__skyDancerV103FinalAudit.setCourse(stageId,progress),{stageId,progress});
  await page.waitForTimeout(100);
  const name=`${String(index).padStart(2,"0")}-${stageId}-${Math.round(progress*100)}-after`;
