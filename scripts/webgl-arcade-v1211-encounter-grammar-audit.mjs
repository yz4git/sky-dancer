...
const out = "artifacts/v1211-encounter-grammar";
...
async function openSortie(tag) {
  ...
  await page.goto(`${baseUrl}?menu=1&v1211=${tag}`, { waitUntil:"domcontentloaded" });
  ...
  await page.waitForFunction(() => Boolean(window.__skyDancerV1211AuditDemo));
  // Freeze only the automatic RAF simulation. Test hooks still mutate the real production runtime,
  // which makes each captured encounter phase deterministic and prevents screenshot time races.
  await page.evaluate(() => { window.__skyDancerV1211AuditDemo.runtime.step = () => {}; });
  return page;
}

async function renderNow(page) {
  await page.evaluate(() => {
    const demo = window.__skyDancerV1211AuditDemo;
...
async function sample(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      beatText:beat?.textContent??"", beatRect:rect(beat), controlsRect:rect(controls),
    };
  });
}
...
function baseChecks(label, state) {
  ...
  const c = state.controlsRect;
  if (!c || c.top < 0 || c.bottom > 393 || c.left < 0 || c.right > 852) report.failures.push(`${label} controls clipped ${JSON.stringify(c)}`);
  ...
  if (b && c && !(b.right <= c.left || b.left >= c.right || b.bottom <= c.top || b.top >= c.bottom)) report.failures.push(`${label} beat overlaps controls`);
}

try {
  const page = await openSortie("sequence");
  await page.evaluate(() => {
    const runtime = window.__skyDancerV1211AuditDemo.runtime;
  ...
  const advanced2 = await page.evaluate(() => window.__skyDancerV1211AuditDemo.runtime.advanceV121EncounterForTests());
  await renderNow(page);
  const second = await sample(page);
  baseChecks("phase2", second);
  if (!advanced2 || second.phaseIndex !== 2 || second.phase === first.phase || second.enemyCount <= first.enemyCount) report.failures.push(`phase2 sequencing ${JSON.stringify({first,second})}`);
  report.phases.push(second);
  await page.screenshot({ path:`${out}/02-dawn-phase-2.png` });

  const advanced3 = await page.evaluate(() => window.__skyDancerV1211AuditDemo.runtime.advanceV121EncounterForTests());
  await renderNow(page);
  const third = await sample(page);
  baseChecks("phase3", third);
  if (!advanced3 || third.phaseIndex !== 3 || third.phase === second.phase || third.enemyCount < second.enemyCount) report.failures.push(`phase3 sequencing ${JSON.stringify({second,third})}`);
...
if (report.failures.length) throw new Error(`V12.1.1 encounter grammar audit failures: ${JSON.stringify(report.failures)}`);
