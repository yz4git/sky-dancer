...
const out = "artifacts/v1141-final-mastery";
...
try {
  ...
  for (const token of ["STAGE MASTERY", "2/3", "BEST A", "NEXT TARGET", "PERFECT SKY"]) if (!menuText.includes(token)) report.failures.push(`menu missing ${token}`);
  ...
  if (flightTelemetry.stage !== "dawn-city" || flightTelemetry.status !== "running" || flightTelemetry.renderer !== "webgl" || flightTelemetry.width !== 852 || flightTelemetry.height !== 393) {
  ...
  const resultPanel = page.locator('[aria-label="Arcade result"]');
  ...
  const resultGeometry = await page.evaluate(() => {
    const box = (el) => el ? (() => { const r = el.getBoundingClientRect(); return { top:r.top, bottom:r.bottom, left:r.left, right:r.right, width:r.width, height:r.height }; })() : null;
    const resultEl = document.querySelector('[aria-label="Arcade result"]');
    ...
    return { result: box(resultEl), mastery: box(masteryEl), retry: box(retryEl), innerWidth, innerHeight, scrollWidth: document.documentElement.scrollWidth, scrollHeight: document.documentElement.scrollHeight };
  });
  ...
  for (const token of ["STAGE MASTERY", "2/3", "NEXT TARGET", "PERFECT SKY", "RETRY STAGE", "CHASE PERFECT SKY"]) if (!resultText.includes(token)) report.failures.push(`result missing ${token}`);
...
if (report.failures.length) throw new Error(`V11.4.1 final mastery audit failures: ${JSON.stringify(report.failures)}`);
