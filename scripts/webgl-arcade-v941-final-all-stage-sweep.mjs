...
const outputDir=process.env.SKY_DANCER_AUDIT_DIR||"artifacts/arcade-v941-final-all-stage-sweep";
const stages=[
...
for(let index=0;index<stages.length;index++){
  ...
  try{
    ...
    diagnostics.push(record);console.log(`[v941-final-sweep] ${stage.id} hp=${hp}`);
...
console.log(`[v941-final-sweep] complete ${diagnostics.length}/${stages.length}`);
