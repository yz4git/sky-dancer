...
export class SkyDancerCanvasPreview implements CartRogueDemoHandle {
  ...
  constructor(
    ...
    this.canvas.className = "cart-rogue-canvas sky-dancer-canvas cart-rogue-canvas-fallback";
    this.canvas.setAttribute("aria-label", "Sky Dancer Canvas fallback");
  ...
  private draw(): void {
    ...
    sky.addColorStop(0, "#65bde8");
    sky.addColorStop(0.58, "#d8f6ff");
    sky.addColorStop(1, "#75c4ec");
    ctx.fillStyle = sky;
    ...
    for (const node of CART_WORLD_GRAPH.nodes) {
      ...
      ctx.fillStyle = node.kind === "corridor" ? "#64c9ea" : node.kind === "boss" ? "#8c6fe8" : "#3b9fd1";
      ctx.fillRect(
      ...
      ctx.fillStyle = "rgba(16, 57, 101, .32)";
      ctx.fillRect(
        p.x - node.rect.halfWidth * scale * 0.82,
        p.y + 1.15 * scale,
        node.rect.halfWidth * 2 * scale * 0.82,
        node.rect.halfDepth * 2 * scale * 0.82,
      );
    ...
    for (const enemy of snapshot.enemies) {
      ...
      const enemyColor = enemy.kind === "boss"
        ? "#65457f"
        : enemy.kind === "heavy"
          ? "#d85f87"
          : enemy.kind === "chaser"
      ...
      ctx.beginPath();
      ctx.moveTo(0, -enemy.radius * 1.3 * scale);
      ctx.lineTo(enemy.radius * 0.52 * scale, enemy.radius * 0.75 * scale);
      ctx.lineTo(0, enemy.radius * 0.38 * scale);
      ctx.lineTo(-enemy.radius * 0.52 * scale, enemy.radius * 0.75 * scale);
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = enemy.kind === "chaser" ? "#b8e879" : enemy.kind === "boss" ? "#f37aa2" : "#ffe879";
      ctx.fillRect(-enemy.radius * 1.18 * scale, enemy.radius * 0.02 * scale, enemy.radius * 2.36 * scale, enemy.radius * 0.2 * scale);
      if (enemy.kind === "boss") {
        ctx.fillStyle = "#f37aa2";
        ctx.fillRect(-enemy.radius * 0.62 * scale, -enemy.radius * 1.72 * scale, enemy.radius * 0.24 * scale, enemy.radius * 0.7 * scale);
        ctx.fillRect(enemy.radius * 0.38 * scale, -enemy.radius * 1.72 * scale, enemy.radius * 0.24 * scale, enemy.radius * 0.7 * scale);
      }
      ctx.restore();
    ...
    const planeScale = Math.max(1, scale * 0.72);
    ctx.fillStyle = snapshot.boostActive ? "#58e5ff" : "#2bbbd9";
    ctx.beginPath();
    ctx.moveTo(0, -2.4 * planeScale);
    ctx.lineTo(0.58 * planeScale, 1.4 * planeScale);
    ctx.lineTo(0, 0.76 * planeScale);
    ctx.lineTo(-0.58 * planeScale, 1.4 * planeScale);
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = "#1d6bb7";
    ctx.beginPath();
    ctx.moveTo(-2.35 * planeScale, 0.15 * planeScale);
    ctx.lineTo(2.35 * planeScale, 0.15 * planeScale);
    ctx.lineTo(0.72 * planeScale, 0.7 * planeScale);
    ctx.lineTo(-0.72 * planeScale, 0.7 * planeScale);
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = "#d9fbff";
    ctx.beginPath();
    ctx.ellipse(0, -0.35 * planeScale, 0.34 * planeScale, 0.62 * planeScale, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "#ffcf67";
    ctx.lineWidth = Math.max(1, scale * 0.12);
    ctx.beginPath();
    ctx.moveTo(-0.18 * planeScale, 1.05 * planeScale);
    ctx.lineTo(-0.18 * planeScale, 1.8 * planeScale);
    ctx.moveTo(0.18 * planeScale, 1.05 * planeScale);
    ctx.lineTo(0.18 * planeScale, 1.8 * planeScale);
    ctx.stroke();
    ctx.restore();
