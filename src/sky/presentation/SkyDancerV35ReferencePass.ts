...
export class SkyDancerV35ReferencePass {
  ...
  private updateFocusCity(snapshot: CartArenaSessionSnapshot): void {
    ...
    // Dense urban fabric begins just ahead of the opening camera and stretches
    // through the midground, matching the reference city-flyover read without
    // attaching scenery to the aircraft each frame.
  ...
  private makeFrontMountainBelt(name: string, count: number, far: boolean): THREE.InstancedMesh {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const height = far
        ? 34 + (index % 6) * 4.0 + Math.sin(index * 1.57) * 3
        : 46 + (index % 6) * 5.0 + Math.sin(index * 1.79) * 4;
