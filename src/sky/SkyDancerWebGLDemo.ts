...
import type { CartArenaSessionSnapshot, CartEnemySnapshot, CartObstacleSnapshot, CartResourceSnapshot } from "../cart/CartArenaSession";
import type { CartRogueDemoHandle, CartRogueSnapshotHandler } from "../cart/CartRogueDemo";
...
export class SkyDancerWebGLDemo implements CartRogueDemoHandle {
  ...
  constructor(
    ...
    this.renderer.domElement.className = "cart-rogue-canvas sky-dancer-canvas";
    this.renderer.domElement.setAttribute("aria-label", "Sky Dancer WebGL game view");
  ...
  /**
   * Cart Rogue's node graph is intentionally kept intact. Only its visual
   * surface is replaced here: each arena/corridor becomes a floating flight
   * deck with an underside, edge rails, and open sky below it.
   */
  private buildAirWorld(): void {
    for (const node of CART_WORLD_GRAPH.nodes) {
      const corridor = node.kind === "corridor";
      const width = node.rect.halfWidth * 2;
      const depth = node.rect.halfDepth * 2;
      const deckColor = node.kind === "boss"
        ? 0x8c6fe8
        : corridor
          ? 0x64c9ea
          : 0x3b9fd1;
      const deck = this.box(width, 0.34, depth, deckColor);
      deck.position.set(node.rect.centerX, -0.32, node.rect.centerZ);
      deck.castShadow = false;
      deck.receiveShadow = true;

      const underside = this.box(width * 0.82, 1.35, depth * 0.82, 0x153965);
      underside.position.set(node.rect.centerX, -1.08, node.rect.centerZ);
      underside.castShadow = true;
      underside.receiveShadow = false;

      const deckInset = this.box(width * 0.92, 0.08, depth * 0.88, corridor ? 0x9eeaff : 0x71d8ff);
      deckInset.position.set(node.rect.centerX, -0.1, node.rect.centerZ);
      deckInset.castShadow = false;
      deckInset.receiveShadow = false;
      this.scene.add(deck, underside, deckInset);

      const railMaterial = this.mat(node.kind === "boss" ? 0xf4a0ff : 0xd5f8ff, node.kind === "boss" ? 0xe778ff : 0x59d9ff);
      for (const side of [-1, 1]) {
        const rail = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.62, depth * 0.9), railMaterial);
        rail.position.set(node.rect.centerX + side * (node.rect.halfWidth - 0.28), 0.18, node.rect.centerZ);
        rail.castShadow = false;
        this.scene.add(rail);
      }

      const stripeCount = Math.max(3, Math.floor(depth / 5));
      for (let index = 0; index < stripeCount; index += 1) {
        const stripe = this.box(width * 0.72, 0.035, 0.11, index % 2 === 0 ? 0xbaf4ff : 0x2d8fca);
        stripe.position.set(
          node.rect.centerX,
          -0.045,
          node.rect.centerZ - node.rect.halfDepth + ((index + 0.5) / stripeCount) * depth,
        );
      ...
      for (const side of [-1, 1]) {
        const beacon = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ color: node.kind === "boss" ? 0xff86ed : 0xffe37c }),
        );
        beacon.position.set(node.rect.centerX + side * (node.rect.halfWidth - 0.62), 0.62, node.rect.centerZ);
  ...
  private buildAircraftVisual(): void {
    this.session.car.group.traverse((object) => { if (object instanceof THREE.Mesh) object.visible = false; });
    this.addContactShadow(this.playerVisual, 1.55, 2.4, 0.18);

    const fuselage = this.taperedBox(0.92, 0.52, 3.35, C.player, 0.62, 0.18);
    fuselage.position.y = 1.14;
    const nose = new THREE.Mesh(new THREE.ConeGeometry(0.48, 1.35, 8), this.mat(C.playerAccent, 0x30d8ff));
    nose.rotation.x = -Math.PI / 2;
    nose.position.set(0, 1.18, 2.28);

    const wings = this.box(4.55, 0.14, 0.76, C.playerDark);
    wings.position.set(0, 1.12, 0.38);
    const wingHighlight = this.box(3.84, 0.07, 0.18, C.playerAccent);
    wingHighlight.position.set(0, 1.2, 0.22);
    const tailplane = this.box(1.82, 0.12, 0.52, C.playerAccent);
    tailplane.position.set(0, 1.25, -1.26);
    const fin = this.taperedBox(0.14, 0.9, 0.72, C.playerAccent, 0.84, 0.06);
    fin.position.set(0, 1.62, -1.12);

    const cockpit = new THREE.Mesh(
      new THREE.SphereGeometry(0.52, 12, 8),
      new THREE.MeshStandardMaterial({ color: 0x9ff3ff, emissive: 0x1684c4, emissiveIntensity: 0.82, roughness: 0.18, metalness: 0.35 }),
    );
    cockpit.scale.set(0.72, 0.44, 1.18);
    cockpit.position.set(0, 1.52, 0.68);

    const noseStripe = this.box(0.18, 0.1, 1.55, C.white);
    noseStripe.position.set(0, 1.44, 1.16);
    const wingTipMaterial = this.mat(0xffd76a, 0xff9b1f);
    const engineMaterial = new THREE.MeshStandardMaterial({ color: 0xff916d, emissive: 0xff3d25, emissiveIntensity: 2.2 });
    for (const x of [-1.88, 1.88]) {
      const wingTip = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.16, 0.7), wingTipMaterial);
      wingTip.position.set(x, 1.18, 0.38);
      this.playerVisual.add(wingTip);
      const engine = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.22, 0.48, 8), engineMaterial);
      engine.rotation.x = Math.PI / 2;
      engine.position.set(x * 0.56, 1.0, -1.7);
      this.playerVisual.add(engine);
    }

    this.playerVisual.add(fuselage, nose, wings, wingHighlight, tailplane, fin, cockpit, noseStripe);
    this.boostLight.position.set(0, 1.0, -2.05);
  ...
  private buildEnemies(enemies: readonly CartEnemySnapshot[]): void {
    for (const enemy of enemies) {
      const group = this.buildAirEnemy(enemy);
  ...
  private buildAirEnemy(enemy: CartEnemySnapshot): THREE.Group {
    const group = new THREE.Group();
    const boss = enemy.kind === "boss";
    const heavy = enemy.kind === "heavy";
    const chaser = enemy.kind === "chaser";
    const scale = boss ? 1.6 : heavy ? 1.25 : 0.96;
    const color = boss ? C.bossEnemy : heavy ? C.heavy : chaser ? C.chaser : C.enemy;
    const accent = boss ? C.bossAccent : chaser ? 0xb8e879 : heavy ? 0xf1b4ff : 0xffe879;
    this.addContactShadow(group, enemy.radius * 1.08, enemy.radius * 1.38, boss ? 0.22 : 0.16);

    const body = this.taperedBox(0.88 * scale, 0.5 * scale, 2.55 * scale, color, 0.62, 0.18);
    body.position.y = 0.98 * scale;
    const nose = new THREE.Mesh(new THREE.ConeGeometry(0.42 * scale, 1.04 * scale, 8), this.mat(accent));
    nose.rotation.x = -Math.PI / 2;
    nose.position.set(0, 1.02 * scale, 1.72 * scale);
    const wings = this.box(3.2 * scale, 0.12 * scale, 0.58 * scale, accent);
    wings.position.set(0, 1.02 * scale, 0.24 * scale);
    const tailplane = this.box(1.35 * scale, 0.1 * scale, 0.4 * scale, color);
    tailplane.position.set(0, 1.11 * scale, -1.04 * scale);
    const fin = this.box(0.13 * scale, 0.7 * scale, 0.52 * scale, accent);
    fin.position.set(0, 1.39 * scale, -0.94 * scale);
    const cockpit = new THREE.Mesh(new THREE.SphereGeometry(0.38 * scale, 10, 6), this.mat(boss ? 0xff8b9a : 0xe8f5ff, boss ? 0xff3f5c : 0x64d8ff));
    cockpit.scale.set(0.72, 0.42, 1.12);
    cockpit.position.set(0, 1.34 * scale, 0.48 * scale);
    group.add(body, nose, wings, tailplane, fin, cockpit);

    const engineMaterial = new THREE.MeshStandardMaterial({ color: 0xff8b62, emissive: 0xff3e20, emissiveIntensity: boss ? 2.6 : 1.8 });
    for (const x of [-1.28, 1.28]) {
      const engine = new THREE.Mesh(new THREE.CylinderGeometry(0.12 * scale, 0.18 * scale, 0.36 * scale, 8), engineMaterial);
      engine.rotation.x = Math.PI / 2;
      engine.position.set(x * scale, 0.87 * scale, -1.28 * scale);
      group.add(engine);
    }

    if (boss) {
      for (const x of [-1, 1]) {
        const horn = new THREE.Mesh(new THREE.ConeGeometry(0.16 * scale, 0.72 * scale, 6), this.mat(C.bossAccent, C.bossAccent));
        horn.position.set(x * 0.68 * scale, 2.02 * scale, 0.12 * scale);
        horn.rotation.z = x * -0.38;
        group.add(horn);
      }
    }

    const hpY = (boss ? 3.0 : heavy ? 2.45 : 2.05) * scale;
    const hpBack = this.box(enemy.radius * 1.9, 0.18, 0.14, C.hpBack);
    hpBack.position.y = hpY;
    const hp = this.box(enemy.radius * 1.76, 0.12, 0.16, C.hp);
    hp.name = "hp-fill";
